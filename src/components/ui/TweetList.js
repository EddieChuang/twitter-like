...
class TweetList extends React.Component {
  ...
  componentWillReceiveProps(props) {
    ...
    let indexToUpdate = tweets.findIndex(
      tweet => tweet._id === tweetToUpdate._id
    )
    ...
    if (indexToUpdate === -1) {
      ...
      tweets = [
        ...tweets.slice(0, indexToUpdate),
        tweetToUpdate,
        ...tweets.slice(indexToUpdate + 1)
      ]
    }
    let matched = tweets
  ...
  filter(e) {
    Array.from(document.getElementsByClassName('btn-filter')).forEach(
      (ele, i) => {
    ...
    return (
      <section id="tweetList">
        <div className="tweetList-header">
          <button className="btn-filter filter-active" onClick={this.filter}>
            All
          </button>
